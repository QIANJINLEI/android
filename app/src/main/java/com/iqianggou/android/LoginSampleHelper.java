...
//    public static final String APP_KEY_TEST = "4272";  //60026702
    ...
    public void initSDK_Sample(Application context) {
        ...
        Log.d("openim","userId = " + userId + ", appkey = " + appkey);
...
//        LocalBroadcastManager.getInstance(YWChannel.getApplication()).sendBroadcast(intent);
    ...
    public void login_Sample(String userId, String password, String appKey,
        ...
        Log.d(TAG, "start login_Sample at " + System.currentTimeMillis());
        if (mIMKit == null) {
            return;
        }
...
//        }
                        ...
                        if (messageBody.getTransparentFlag() == 1) {
                            ...
                            try {
                                ...
                                if (object.has("text")) {
                                    ...
                                    IMNotificationUtils.getInstance().showToast(AiQGApplication.getContext(), "透传消息，content = " + text);
    ...
    public static final String SERVICE_NAME = "iqg7788";
    public void dochatWithServer(Context context) {
        Log.d(TAG, "start dochatWithServer.");
        if (mIMKit == null){
            return;
        }
        EServiceContact contact = new EServiceContact(SERVICE_NAME,157450847);//第一个参数就是淘宝客服帐号，第二个参数是分流组，附链接。
        Intent intent = mIMKit.getChattingActivityIntent(contact);
        context.startActivity(intent);
        Log.d(TAG, "start dochatWithServer activity.");

//        Intent intent =AiQGApplication.getInstance().getYwImKit().getChattingActivityIntent("xuexi");
//        context.startActivity(intent);
    }
}
